<template>
  <div>
    ...
    <div v-else style="line-height: 30px; margin-right: 20px; border: none; display: inline-block">
      <el-button
        size="small"
        icon="el-icon-bell"
    ...
    <el-dialog title="新消息提示" :visible.sync="showMailBox">
      <el-collapse accordion @change="readMail">
        <el-collapse-item v-for="mail in mails" :name="mail.pk" :key="mail.pk">
          <template slot="title">
            <div :style="{ width: '96%', color: !mail.is_read ? '#2d8f6e' : 'inherit' }">
              <i class="el-icon-chat-line-round" style="margin: 0 8px; font-size: 16px"></i>{{ mail.title }}
              <div style="font-size: 14px; float: right">{{ mail.is_read ? '' : '（未读）' }}{{ formatTheDate(mail.create_time) }}</div>
            </div>
          </template>
      ...
      <div style="text-align: center; margin: 30px 0 10px 0">
        <el-button type="primary" round size="medium" @click="showMailBox = false">我知道了~</el-button>
      </div>
    </el-dialog>
  </div>
</template>
