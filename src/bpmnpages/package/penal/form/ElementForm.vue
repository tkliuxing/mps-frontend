  ...
  <div class="panel-tab__content">
    ...
    <div class="element-property list-property">
      ...
      <el-table :data="fieldList" size="mini" max-height="240" border fit>
        <el-table-column label="序号" type="index" width="45px"/>
        ...
        <el-table-column label="是否必填" prop="is_required" min-width="50px" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <el-tag size="mini" type="success" v-if="row.is_required == 'true'">是</el-tag>
    ...
    <el-drawer :visible.sync="fieldModelVisible" title="字段配置" :size="`${width}px`" append-to-body destroy-on-close>
      <el-form :model="formFieldForm" label-width="90px" size="mini" @submit.native.prevent>
        <el-form-item label="字段ID">
          <el-select v-model="formFieldForm.id" clearable @change="changeFieldId">
            <el-option v-for="i in formtemplate.field" :key="i.pk" :value="i.alias || i.col_name">{{ i.col_title }}</el-option>
          </el-select>
        ...
        <el-form-item label="是否必填">
          <el-switch
            style="display: block"
            v-model="formFieldForm.is_required"
            active-color="#13ce66"
...
export default {
  ...
  methods: {
    ...
    // 选择字段
    changeFieldId(pk) {
      this.$set(this.formFieldForm, "label", this.formtemplate.field.filter(i => i.col_name == pk || i.alias == pk)[0].col_title);
    },
    // 打开字段详情侧边栏
    openFieldForm(field, index) {
      ...
      if (index !== -1) {
        ...
        // this.formFieldForm.typeType = 'string';
        // 设置自定义类型
    ...
    // 保存字段配置
    saveField() {
      const {id, type, label, defaultValue, datePattern, is_required } = this.formFieldForm;
      const Field = window.bpmnInstances.moddle.create(`${this.prefix}:FormField`, {id, type, label});
      defaultValue && (Field.defaultValue = defaultValue);
      datePattern && (Field.datePattern = datePattern);
      is_required && (Field.is_required = is_required);
      // 构建属性
